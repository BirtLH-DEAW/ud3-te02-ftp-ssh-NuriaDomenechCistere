...
# Actualizamos la lista de paquetes e instalamos nano, apache2 y proftpd
RUN apt-get update && \
...
# Agregar configuraciones a proftpd.conf 
RUN echo "Include /etc/proftpd/conf.d/" >> /etc/proftpd/proftpd.conf && \
    ...
    echo "</Limit>" >> /etc/proftpd/proftpd.conf
# Agregar la configuración al archivo /etc/proftpd/tls.conf
...
# Establecemos permisos seguros para las claves privadas
